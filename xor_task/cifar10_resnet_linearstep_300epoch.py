...
RESULTS = os.path.join(os.getcwd(), "results", "RESNET_LINEARSTEP_DEVICE_300EPOCH")
...
DEVICE_CONFIG_PATH = os.path.join(os.path.dirname(__file__), 'Current_LinearStepDevice_with_variation_config.json')
...
print(f"Loaded fitted LinearStepDevice config from {DEVICE_CONFIG_PATH}:")
...
def main():
    ...
    print(f"\nUsing fitted LinearStepDevice for analog layers:")
    print(f"  dw_min={device_config.get('dw_min', 'N/A'):.6f}")
    print(f"  gamma_up={device_config.get('gamma_up', 'N/A'):.6f}")
    print(f"  gamma_down={device_config.get('gamma_down', 'N/A'):.6f}")
    print(f"  Cycle-to-cycle variation: gamma_up_dtod={device_config.get('gamma_up_dtod', 'N/A'):.4f}, gamma_down_dtod={device_config.get('gamma_down_dtod', 'N/A'):.4f}")
    ...
    global WEIGHT_PATH
    WEIGHT_PATH = os.path.join(RESULTS, f"linearstep_device_model_weight_{N_EPOCHS}epoch.pth")
    ...
    for epoch in epoch_pbar:
        ...
        if latest_val_acc > best_accuracy:
            ...
            best_epoch = epoch
            # Save best model
            best_model_path = os.path.join(RESULTS, f"linearstep_device_best_model_{N_EPOCHS}epoch.pth")
    ...
    plt.suptitle(f'Fitted LinearStepDevice ResNet18 CIFAR10 ({N_EPOCHS} epochs)\n'
    ...
    plt.suptitle(f'Feature Embeddings - Fitted LinearStepDevice ResNet18\nVal Acc: {best_accuracy:.2f}%', fontsize=14)
    plt.tight_layout()
    ...
    plt.suptitle(f'Confusion Matrix - Fitted LinearStepDevice ResNet18\nVal Acc: {best_accuracy:.2f}%', fontsize=14)
    plt.tight_layout()
    ...
    # Summary report
    summary = {
        'experiment': 'Fitted LinearStepDevice ResNet18 CIFAR10',
        'epochs': N_EPOCHS,
        ...
        'device_config': {
            ...
            'gamma_up': device_config.get('gamma_up'),
            'gamma_down': device_config.get('gamma_down'),
