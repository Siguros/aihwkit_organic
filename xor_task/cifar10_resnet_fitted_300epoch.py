...
"""aihwkit baseline: ResNet18 CNN with CIFAR10 using Fitted PiecewiseStepDevice.

CIFAR10 dataset on a ResNet18 network with configurable digital (FloatingPoint)
and analog (PiecewiseStepDevice) layers. Uses fitted device from xor_device_config.json.
"""
...
RESULTS = os.path.join(os.getcwd(), "results", "RESNET_FITTED_DEVICE_300EPOCH")
...
NESTEROV = True  # Nesterov momㅉentum
...
print(f"  write_noise_std: {device_config.get('write_noise_std', 'N/A')}")
...
def main():
    ...
    global WEIGHT_PATH
    WEIGHT_PATH = os.path.join(RESULTS, f"fitted_device_model_weight_{N_EPOCHS}epoch.pth")

    print("\nStarting Fitted PiecewiseStepDevice training on CIFAR10...")
    print("=" * 60)
    ...
    for epoch in epoch_pbar:
        ...
        if latest_val_acc > best_accuracy:
            ...
            best_epoch = epoch
            # Save best model
            best_model_path = os.path.join(RESULTS, f"fitted_device_best_model_{N_EPOCHS}epoch.pth")
    ...
    plt.suptitle(f'Fitted PiecewiseStepDevice ResNet18 CIFAR10 ({N_EPOCHS} epochs)\n'
    ...
    plt.suptitle(f'Feature Embeddings - Fitted Device ResNet18\nVal Acc: {best_accuracy:.2f}%', fontsize=14)
    plt.tight_layout()
    ...
    plt.suptitle(f'Confusion Matrix - Fitted Device ResNet18\nVal Acc: {best_accuracy:.2f}%', fontsize=14)
    plt.tight_layout()
    ...
    # Summary report
    summary = {
        'experiment': 'Fitted PiecewiseStepDevice ResNet18 CIFAR10',
        'epochs': N_EPOCHS,
